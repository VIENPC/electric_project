...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public TokenBasedRememberMeServices rememberMeServices() {
		TokenBasedRememberMeServices rememberMeServices = new TokenBasedRememberMeServices("uniqueAndSecretKey",
				userDetailsService);
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests(requests -> requests
				.antMatchers("/assets/**", "/", "logout", "/login**", "/home", "/shop", "/error**", "/api/**",
						"/reset-password", "/codeVerification", "/resendOtp", "/new-password", "/rest/productdetails",
						"/rest/products", "/rest/productsbycate/**", "/rest/products/**", "/rest/**", "/product",
						"/registration/**", "/oauth2/**")
				.permitAll().antMatchers("/admin/**", "/rest/orders/**").hasRole("ADMIN").anyRequest().authenticated())
				...
				.rememberMe(rememberMe -> rememberMe.key("uniqueAndSecretKey").rememberMeServices(rememberMeServices())
				...
				.oauth2Login(t -> t.loginPage("/login").failureUrl("/login?error=true").userInfoEndpoint()
						.userService(oAuth2UserService).and().successHandler(oAuth2LoginSuccessHandler).permitAll()
						.permitAll())
				.logout(logout -> logout.logoutSuccessUrl("/home").logoutSuccessHandler(new LogoutSuccessHandler() {
					@Override
